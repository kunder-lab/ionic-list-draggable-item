...
'use strict';
  ...
  HoldOp.prototype.startTimer = function() {
    var self = this;

    self.holdTimeout = _createHoldTimeout(self);
  };

  HoldOp.prototype.clearTimer = function() {
    var self = this;
    
    if(self.holdTimeout && self.holdTimeout !== null) {
      clearTimeout(self.holdTimeout);
      self.holdTimeout = null;
    }
  };

  var _createHoldTimeout = function(_holdOp) {
    // This timer is created to "free" the holded item in case it's not dragged around
    return setTimeout(function() {
      _holdOp.holdedItem.holded = false;
    }, 1000);
  };
   ...
   */
  ionic.views.DraggableItemListView = ionic.views.View.inherit({
    initialize: function(opts) {
      ...
      window.ionic.onGesture('hold', function(e) {
        self._handleHold(e);
      }, self.el, gestureOpts);
    ...
    _startDrag: function(e) {
      ...
      if (e.gesture.direction == 'up' || e.gesture.direction == 'down') {
        var target = e.target || e.simulatedTarget;
        item = self._getItem(target);

        if (item && (item.holded || ionic.DomUtil.getParentOrSelfWithClass(target, ITEM_REORDER_BTN_CLASS))) {
          ...
          // Freeze scroll during drag event
    ...
    _startHold: function(e) {
      ...
      // new hold operation
      self._holdOp = new HoldOp();
      var item = self._getItem(e.target);

      if(item) {
        item.holded = true;
        self._holdOp.holdedItem = item;
        self._holdOp.startTimer();

        // Recreation of drag event so the element moves a few pixels and shows the user it can move the item around
        var dragEvent = new ionic.CustomEvent('drag', e);
        dragEvent.simulatedTarget = e.target;
        dragEvent.gesture = {};

        ionic.extend(dragEvent.gesture, e.gesture, {
          angle: 90,
          deltaTime: 1000,
          deltaX: 0,
          deltaY: 10,
...
angular.module('kunder.ionicListDraggableItem', [])
...
function($timeout) {
  ...
  return {
    restrict: 'E',
    require: ['listDraggableItem', '^?$ionicScroll'],
    controller: 'ionicListDraggableItemController',
    compile: function($element, $attr) {
      var jqLite = angular.element;

      var listEl = jqLite('<div class="list draggableItem">')
      ...
      return function($scope, $element, $attrs, ctrls) {
        ...
        function init() {
          var listView = listCtrl.listView = new ionic.views.DraggableItemListView({
            el: $element[0],
